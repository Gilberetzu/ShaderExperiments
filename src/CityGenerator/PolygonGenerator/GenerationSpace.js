...
export default class GenerationSpace{
		...
		this.vertices = vertices.map(vert => {
			...
			if(!vert.outer){
				...
				for (let i = 0; i < orderedEdges.length; i++) {
					...
					adjacentVertices.push(vertices.findIndex(v => v == oEdge.tvert));
	...
	addSpaceToScene(scene){
		/*this.vertLayersInstancedMeshes.forEach(layerInstanceMesh => {
			scene.add(layerInstanceMesh);
		})*/
	...
	raycastToSpace(ray){
		...
		const ray2dOrigin = new Vec2(ray.origin.z, ray.origin.x);
	...
	updateSpaceGeometry(modelManager){
		...
		for (let layerIndex = 0; layerIndex < maxVerticalLayers - 1; layerIndex++) { //layer loop
			for (let prismIndex = 0; prismIndex < this.triangles.length; prismIndex++) { //prism loop
				...
				for (let modelIndex = 0; modelIndex < modelManager.models.length; modelIndex++) {
					const model = modelManager.models[modelIndex];
